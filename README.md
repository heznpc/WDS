...
- 항목별로 **자동 정리**를 켤 수 있습니다(아이폰 텍스트 대치처럼 확인 없이 적용). 자동이어도 위의 재검증 전체를 동일하게 수행하고 사용자 확인 단계만 생략하며, 등록 시 바꿀 말이 찾을 문구를 포함하면 거부하고 포커스 세션당 자동 적용 횟수를 제한해 무한 반복을 차단합니다. 자동 항목이 여러 번 매칭되면 편집 후 새 스냅샷에서 다음 매칭이 이어서 정리됩니다.
