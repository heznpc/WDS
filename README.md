...
메뉴 맨 위의 **입력 정리**와 **출처 구분**은 독립 스위치입니다. 입력 정리만 켜면 군더더기·표기 오류 후보를 제안하고, 출처 구분만 켜면 초안을 감시하지 않고 명시적으로 가져온 외부 의견에 경계·검토 지침을 붙입니다. 둘 다 켜거나 둘 다 끌 수도 있습니다. 각 설정은 재실행 후에도 유지되며, 기존 엔진 설정은 두 스위치의 초기값으로 이전됩니다. 입력 정리를 꺼도 앱별 감시 동의는 보존되어 다시 켤 때 복구됩니다.
...
출처 구분은 입력 정리의 앱별 감시 허용과 무관하게 사용할 수 있습니다. 손쉬운 사용 권한은 필요하지만, 한 번 가져오기를 실행해도 그 앱의 초안 감시를 켜거나 감시 허용 목록에 추가하지 않습니다. 출처 구분을 꺼도 이미 입력한 경계와 원문은 유지되며, 입력 정리는 기존 경계 안의 인용문을 계속 보호합니다.

입력 정리는 메뉴바에서 `입력 정리`를 켠 뒤 `현재 앱 입력 정리 시작…`을 선택하면 권한·앱 자동 감시 허용이 순서대로 안내됩니다. 저장되는 것은 앱 식별자뿐이며, 다음 WDS 실행이나 대상 앱 재실행 뒤에도 그 앱이 전면에 오면 감시가 자동 재개됩니다. 문장을 입력하고 약 0.4초 멈춘 뒤 입력창 옆 후보에서 `날리기`를 누르거나 `⌃⌘⌫`를 눌러야 실제로 사라집니다. `⌃⌘K`는 현재 후보를 유지합니다. 두 전역 단축키는 후보가 보이는 동안에만 등록되며 WDS는 Enter나 전송 버튼을 누르지 않습니다.
...
- `현재 앱 입력 정리 시작…`에서 앱별 자동 감시를 명시적으로 허용하며, `중지`하면 그 지속 동의도 함께 해제
